...
export function RadialOrbitalTimeline({ items }) {
  ...
  return (
    ...
    >
      <div className="radial-container max-w-6xl mx-auto p-4 md:p-8 flex flex-col md:flex-row items-center gap-12 bg-black/95 rounded-3xl border border-[#ff4500]/20 shadow-2xl overflow-hidden relative min-h-[600px]">
        
        {/* Ambient background glow */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#ff4500]/5 via-transparent to-[#1a0800]/50 pointer-events-none" />
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-[500px] height-[500px] bg-[#ff4500]/10 rounded-full blur-[120px] pointer-events-none" />
        ...
        <div className="relative w-full md:w-1/2 h-[600px] flex items-center justify-center">
          ...
          {/* Core center point */}
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-[#ff4500] rounded-full shadow-[0_0_20px_#ff4500] z-20">
            <div className="absolute inset-0 bg-[#ff4500] rounded-full animate-ping opacity-50" />
            <div className="absolute w-[200px] h-[1px] bg-gradient-to-r from-[#ff4500] to-transparent left-full top-1/2 -translate-y-1/2 opacity-30" />
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#ff4500]/10 border border-[#ff4500]/20 text-[#ff4500] text-xs font-bold uppercase tracking-widest w-fit mb-6">
                <Sparkles size={14} />
                {activeItem?.type || "Node Active"}
              </div>

              {/* Title & Desc */}
              <h2 className="text-4xl md:text-5xl font-['Cinzel'] font-bold text-white mb-6 leading-tight text-shadow-sm shadow-[#ff4500]/20">
                {activeItem?.title || activeItem?.name || activeItem?.role}
              </h2>
              ...
              <div className="flex flex-wrap gap-2 mb-10">
                {activeItem?.technologies?.split('•').map((tech, i) => (
                  <span 
                    key={i} 
                    className="px-3 py-1 bg-white/5 border border-white/10 rounded-md text-zinc-300 text-xs font-medium"
                  >
                    {tech.trim()}
                  </span>
                ))}
              ...
              {activeItem?.projectUrl && (
                ...
                >
                  <span className="font-bold tracking-wide uppercase text-sm">Deploy Connect</span>
